...
export async function prepareSecret() {
  ...
  var url = `https://developer.api.autodesk.com/applications/v1/clients/${CLIENT_ID}/secret:prepare`;
...
export async function commitSecret(newSecret) {
  const accessToken = await getAccessToken();
  const url = `https://developer.api.autodesk.com/applications/v1/clients/${CLIENT_ID}/secret:commit`;
...
export async function cancelSecret() {
  const accessToken = await getAccessToken();
  const url = `https://developer.api.autodesk.com/applications/v1/clients/${CLIENT_ID}/secret:cancel`;
  const clientAssertion = await sign({
    iss: CLIENT_ID,
    sub: CLIENT_ID,
    aud: url,
